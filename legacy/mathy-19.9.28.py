...
def print_matrix(A,precision=2):
    ...
    for i in range(len(A)):
        ...
        print()
    print(']') 
def print_vector(A,precision=2):
...
def dot(a,b):
    if len(a)!=len(b):
        raise ValueError("a,b should have the same length!")
    s = 0
    for i in range(len(a)):
        s+=a[i]*b[i]
    return s
# Gram Schmidt Orthogonalization 接收的是矩阵，一般来讲，向量组中向量的个数应等于向量的维数   
def schmidt_matrix(A):
    if len(A)>len(A[0]):
        raise ValueError("num of vectors should be at least equal to the dimension of vectors! (make sure cols>=rows)")
    #orth_vec_list = zeros(len(A,len(A[0]))) # 生成equivalent转置大小的矩阵便于进行内积运算
    col_vec_group= split_col(A)
    orth_vec_group = zeros(1,len(A[0]))
    
    
        
    
def schmidt_vec_group(A):
    pass
def schmidt(A):
    #自适应传入矩阵和向量组
    pass
...
# 约定：矩阵是二维list，向量也是二维list，向量组是在tuple中的几个向量（tuple套着几个二维list）

...
def integrate_regular(function,start,end,precision = 9000):
	sum = 0
...
# 接收row_echelon的矩阵，找到pivot的位置，返回几个下标的tuple
def find_pivot(A):
...
def rref(A):
    ...
    for i in range(len(index_list)):
        multiply_row(B,index_list[i][0],1/B[index_list[i][0]][index_list[i][1]]) # 这里偶尔会出现除以0的错误，可以用随机阵暴力测试，研究原因
...
if __name__ == '__main__':
    ...
    #A 
    A = [[1,1,1,1],[2,1,0,1],[1,2,3,4]]
    #print_matrix(A)
    #row_echelon_display_process(A)
    #print(det(C))
    #print(eigen_polynomial(A))
    ############## 还需要一个正交化的程序
    ####### rref 还有bug，需要随机矩阵暴力测试，看看和matlab结果哪里不同
    ###需要先写一个随机矩阵生成器，可以满足一定的秩，正交的需求
